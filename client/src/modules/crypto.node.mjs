...
export const hash = async (algorithm, data) => {
  ...
  assert(algorithms.has(algorithm) === true);
 ...
 * @param {number} digits
 * @param {number} period
 * @param {number} look_ahead_window
 * @returns {Promise<string>}
 */
export const totp_code = async (key, algorithm, digits, period, look_ahead_window) => {
  assert(typeof key === 'string');
  assert(typeof algorithm === 'string');
  assert(algorithms.has(algorithm) === true);
  assert(typeof digits === 'number');
  assert(typeof period === 'number');
  assert(typeof look_ahead_window === 'number');
  const counter = totp_counter(period) + look_ahead_window;
